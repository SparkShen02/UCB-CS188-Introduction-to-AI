...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        predicted_y = nn.AddBias(nn.Linear(x, self.w1), self.b1)
        x = nn.ReLU(predicted_y)
        predicted_y = nn.AddBias(nn.Linear(x, self.w2), self.b2)
        x = nn.ReLU(predicted_y)
        predicted_y = nn.AddBias(nn.Linear(x, self.w3), self.b3)
        return predicted_y
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(200):
            if dataset.get_validation_accuracy() > 0.976:
                ...
                loss = self.get_loss(x, y)
                grad_wrt_w1, grad_wrt_b1, grad_wrt_w2, grad_wrt_b2, grad_wrt_w3, grad_wrt_b3 = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
                self.w1.update(grad_wrt_w1, -0.3)
                self.b1.update(grad_wrt_b1, -0.3)
                self.w2.update(grad_wrt_w2, -0.3)
                self.b2.update(grad_wrt_b2, -0.3)
                self.w3.update(grad_wrt_w3, -0.3)
                self.b3.update(grad_wrt_b3, -0.3)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                h = nn.AddBias(nn.Linear(xs[i], self.w1), self.b1)
                x = nn.ReLU(h)
                h = nn.AddBias(nn.Linear(x, self.w2), self.b2)
            else:
                h = nn.AddBias(nn.Add(nn.Linear(xs[i], self.w1), nn.Linear(h, self.w1_hidden)), self.b1_hidden)
                x = nn.ReLU(h)
                # h = nn.AddBias(nn.Add(nn.Linear(x, self.w2), nn.Linear(x, self.w2_hidden)), self.b2_hidden)
                h = nn.AddBias(nn.Linear(x, self.w2_hidden), self.b2_hidden)
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(100):
            if dataset.get_validation_accuracy() > 0.88:
                ...
                loss = self.get_loss(x, y)
                grad_wrt_w1, grad_wrt_b1, grad_wrt_w2, grad_wrt_b2, grad_wrt_w1_hidden, grad_wrt_b1_hidden, grad_wrt_w2_hidden, grad_wrt_b2_hidden, grad_wrt_w3, grad_wrt_b3 = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2, self.w1_hidden, self.b1_hidden, self.w2_hidden, self.b2_hidden, self.w3, self.b3])
                self.w1.update(grad_wrt_w1, -0.2)
                self.b1.update(grad_wrt_b1, -0.2)
                self.w2.update(grad_wrt_w2, -0.2)
                self.b2.update(grad_wrt_b2, -0.2)
                self.w1_hidden.update(grad_wrt_w1_hidden, -0.2)
                self.b1_hidden.update(grad_wrt_b1_hidden, -0.2)
                self.w2_hidden.update(grad_wrt_w2_hidden, -0.2)
                self.b2_hidden.update(grad_wrt_b2_hidden, -0.2)
